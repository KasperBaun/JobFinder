...
- **R-076** The system should, on first run, guide the user to create their own profile rather than seeding a generic example; the profile step is skippable, and while no profile exists the Profile and Search surfaces should prompt the user to finish it instead of failing.
