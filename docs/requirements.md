...
- **R-089** The system should support per-portal body enrichment (`enrichBody: true` in the catalog): after producing catalog listings, fetch each listing's linked HTML page with a bounded concurrency (and a per-fetch stagger so no single host is hammered) and merge the visible text into the listing's description before ranking; output order matches input order. Failures don't drop the listing — the catalog-only version is kept and the failure logged.
