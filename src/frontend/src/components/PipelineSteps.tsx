...
type StepKey = keyof Messages['search']['steps']
type Step = { phase: JobSearch['phase']; key: StepKey }

const STEPS: Step[] = [
  { phase: 'fetching', key: 'fetching' },
  { phase: 'deduping', key: 'deduping' },
  { phase: 'ranking', key: 'ranking' },
  { phase: 'llmJudging', key: 'llmJudging' },
  { phase: 'done', key: 'done' },
]
...
export function PipelineSteps({ phase, state, aiEnabled, timeline, startedAt, finishedAt, currentAttemptStartedAt }: Props) {
  ...
  return (
    <div className="pipeline" role="list" aria-label={t.stepsAria}>
      {steps.map((step, i) => {
        ...
        return (
          <div key={step.phase} className={`pipeline__step pipeline__step--${mod}`} role="listitem">
            ...
            <span className="pipeline__label">{t.steps[step.key]}</span>
