...
1. **Log** via your handler base class's `ExecuteAsync()` wrapper
...
| MUST | MUST NOT |
|------|----------|
| Inherit your handler base class | Inject `DbContext` |
    ...
    ICustomLogger logger) : HandlerBase(logger), IOrderHandler
{
    ...
    public Task<IResult> GetOrderById(ClaimsPrincipal claimsPrincipal, Guid orderId)
        => ExecuteAsync(
            "get order: {OrderId}",
            async () =>
            {
...
The base `ExecuteAsync()` wrapper on your handler base class provides consistent logging and exception handling:
...
Exceptions are handled centrally in the handler base class, **not** in individual handlers or middleware. Handlers do **NOT** catch exceptions.
...
```csharp
public Task<IResult> ApproveOrder(ClaimsPrincipal claimsPrincipal, Guid orderId)
    => ExecuteAsync(
...
public interface IOrderHandler
{
    ...
    Task<IResult> GetOrderById(ClaimsPrincipal claimsPrincipal, Guid orderId);
...
public async Task<IResult> GetOrder(ClaimsPrincipal user, Guid id)
{
    try { ... }
