...
private async Task WaitForStateTransition<T>(Guid entityId, Func<T, bool> predicate, int timeoutMs = 5000) where T : class
{
...
- Run integration tests against a real database (an embedded server or a disposable container), not EF InMemory — so you exercise FK cascades, unique indexes, real SQL translation, decimal precision, etc. Use a uniquely-named database per factory for isolation.
- Write tests expecting relational behaviour, not InMemory leniency.
...
- [unit-tests.md](unit-tests.md) - Unit test patterns and hand-rolled fakes
